...
export class CustomersDeleteComponent {
    ...
    deleteResource() {
        this.customersService.deleteResourceById(this.id)
            .subscribe(() => {
                this.onBack();
                this.notifications.createNotification('success', 'SUCCESS', 'customers.successDeleteCustomer');
            }, err => {
                console.error(err);
                this.notifications.createNotification('error', 'ERROR', 'customers.errorDeleteCustomer');
            });
    }
}
